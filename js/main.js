...
//     }, 4000);  // Switches text every 4 seconds (same as the animation duration)
// });
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let typing = true; // To keep track if we're typing or erasing
    let textIndex = 0;
    let charIndex = 0;
    let interval;
    ...
    function erase() {
        ...
        if (charIndex > 0) {
            ...
            currentTextIndex = (currentTextIndex + 1) % texts.length; // Switch text
            setTimeout(type, 500); // Wait 0.5 second before starting to type the next text
        }
    }

    // Start typing the first text immediately
    interval = setInterval(type, 200); // Type character every 200ms
});
